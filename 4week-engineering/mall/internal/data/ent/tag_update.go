...
// AddPosts adds the "posts" edges to the Product entity.
func (tu *TagUpdate) AddPosts(p ...*Product) *TagUpdate {
...
// ClearPosts clears all "posts" edges to the Product entity.
func (tu *TagUpdate) ClearPosts() *TagUpdate {
	tu.mutation.ClearPosts()
	return tu
}

// RemovePostIDs removes the "posts" edge to Product entities by IDs.
func (tu *TagUpdate) RemovePostIDs(ids ...int64) *TagUpdate {
	tu.mutation.RemovePostIDs(ids...)
	return tu
}

// RemovePosts removes "posts" edges to Product entities.
func (tu *TagUpdate) RemovePosts(p ...*Product) *TagUpdate {
	ids := make([]int64, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
// AddPosts adds the "posts" edges to the Product entity.
func (tuo *TagUpdateOne) AddPosts(p ...*Product) *TagUpdateOne {
	ids := make([]int64, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
// RemovePosts removes "posts" edges to Product entities.
func (tuo *TagUpdateOne) RemovePosts(p ...*Product) *TagUpdateOne {
	ids := make([]int64, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
